...
class AmplificationPluginRunner(Plugin):
    ...
    def handle_packet(self, packet):
        ...
        self.logger.debug("[%s] Received packet (%d): %s / %s", self.strategy_ports[strategy_port].environment_id,
                          len(bytes(packet)), packet.summary(), packet["TCP"].payload)
    ...
    def evaluate(self, args, evaluator, population, logger):
        ...
        sniffer = AsyncSniffer(filter="tcp and src port %d" %
                               dport, prn=self.handle_packet, store=False)
        sniffer.start()
        ...
        for ind in tqdm.tqdm(population, leave=False, disable=(actions.utils.CONSOLE_LOG_LEVEL == "debug")):
            ...
            ack = int(RandInt())
            packets = [
                IP(dst=dst)/TCP(sport=sport, dport=dport,
                                flags="PA", ack=ack, seq=seq+1)/Raw(payload)
            ]
            ...
            for packet in packets_to_send:
                ...
                self.logger.debug("Sending packet (%d) %s",
                                  len(bytes(packet)), str(packet))
                # Send the packet
        ...
        logger.info(
            "Sleeping %d cooldown seconds to wait for packets to come in" % args["cooldown"])
        ...
        for port in self.strategy_ports:
            ...
            if ind.fitness != -1000:
                ...
                if port in self.responses:
                    ...
                    self.logger.debug("[%s] Fitness %s: %s" % (
                        ind.environment_id, ind.fitness, str(ind)))

                ind.fitness = actions.utils.punish_unused(
                    ind.fitness, logger, ind)
            logger.debug("[%s] Fitness: %s: %s" %
                         (ind.environment_id, ind.fitness, str(ind)))
    ...
    def get_args(command):
        ...
        parser = argparse.ArgumentParser(
            description='Amplification plugin runner', allow_abbrev=False)
        parser.add_argument('--output-directory',
                            action='store', help="Where to output results")
        parser.add_argument('--sleep', action='store', type=int, default=500,
                            help='milliseconds to sleep between each strategy')
        parser.add_argument('--port', action='store',
                            type=int, default=7, help='port to use')
        parser.add_argument('--dst', action='store', help='IP to use')
        parser.add_argument('--runs', action='store', help='Runs per strategy')
        parser.add_argument('--site', action='store', default="pornhub.com",
                            help='Site to include in the HTTP GET request')
        parser.add_argument('--disregard-empty', action='store_true',
                            help='Disregard packets without payloads (RSTs)')
        parser.add_argument('--cooldown', action='store', type=int, default=8,
                            help='amount of time after the last packet is sent to collect packets')
